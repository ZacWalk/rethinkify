...
pf::file_path pf::file_path::app_data_folder()
{
	wchar_t raw_path[MAX_PATH];
	SHGetSpecialFolderPathW(GetActiveWindow(), raw_path, CSIDL_LOCAL_APPDATA | CSIDL_FLAG_CREATE, TRUE);

	auto result = file_path(utf16_to_utf8(raw_path)).combine(g_app_name);
	const auto w_result = utf8_to_utf16(result.view());
	const auto attribs = GetFileAttributesW(w_result.c_str());
	if (attribs == INVALID_FILE_ATTRIBUTES)
		CreateDirectoryW(w_result.c_str(), nullptr);
	return {result};
}
...
static pf::file_path get_config_path()
{
	...
	return pf::file_path::app_data_folder().combine(u8"rethinkify", u8"ini");
}
...
public:
	win_spell_checker()
	{
		...
		_custom_dic_path = pf::file_path::app_data_folder().combine(u8"custom.dic").view();
